    ...
    const handleSubscribe = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.status === 'subscribed') {
                ...
                setStatus('error');
                setMessage(t('newsletter.error_message'));
            }
        } catch {
            setStatus('error');
            setMessage(t('newsletter.error_connection'));
        }
    };
                ...
                {/* Decorative header ornament */}
                    <div className="flex items-center justify-center gap-4 mb-8">
                        ...
                        <div className="flex items-center gap-2 text-[#c9a84c]">
                            <Sparkles size={14} />
                            <span className="text-[10px] font-semibold tracking-[5px] uppercase">{t('newsletter.inner_circle')}</span>
                            <Sparkles size={14} />
                        </div>
                ...
                {(status === 'subscribed' || status === 'already_subscribed') && (
                    <div className={`relative overflow-hidden rounded-2xl p-8 border shadow-sm text-center transition-all duration-500 ${
                        ...
                        <p className={`font-serif text-xl font-semibold mb-2 ${status === 'subscribed' ? 'text-[#c9a84c]' : 'text-[#1e1b14]'}`}>
                            {status === 'subscribed' ? t('newsletter.welcome_title') : t('newsletter.already_member')}
                        </p>
                        ...
                        <div className="mt-2">
                            <button onClick={reset} className="text-sm text-[#c9a84c] hover:text-[#e8c96e] underline transition-colors cursor-pointer">
                                {status === 'subscribed' ? t('newsletter.dismiss') : t('newsletter.go_back')}
                            </button>
                        </div>
                    </div>
                )}
