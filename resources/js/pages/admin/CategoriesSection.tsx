    ...
    return (
        <>
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h2 className="font-serif text-[32px] font-bold m-0 leading-tight text-foreground">Categories</h2>
                    <p className="text-sm mt-1.5 text-admin-muted">Organise your products with categories.</p>
                </div>
            ...
            <div className="bg-white rounded-xl overflow-hidden border border-admin-border">
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {categories.length === 0 && (
                            <tr><td colSpan={3} className="px-6 py-14 text-center text-sm text-admin-faint">No categories yet. Create your first one!</td></tr>
                        )}
                        {categories.map((cat) => (
                            <tr key={cat.id} className="transition-colors duration-150 hover:bg-[#fdf9f2] border-b border-admin-border last:border-b-0">
                                ...
                                <td className="px-5 py-4 text-sm text-admin-muted">{cat.products_count ?? 0} product{(cat.products_count ?? 0) !== 1 ? 's' : ''}</td>
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
                    ...
                    <div className="relative z-10 w-full max-w-[480px] rounded-2xl overflow-hidden bg-card border border-admin-border shadow-[0_40px_100px_rgba(0,0,0,0.18)]">
                        <div className="flex items-center justify-between shrink-0 px-7 py-6 bg-[#faf8f4] border-b border-admin-border">
                            <h3 className="font-serif text-[22px] font-bold m-0 text-foreground">{editing ? 'Edit Category' : 'Add Category'}</h3>
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
                    ...
                    <div className="relative z-10 w-full max-w-[440px] rounded-2xl p-8 bg-card border border-admin-border shadow-[0_40px_100px_rgba(0,0,0,0.18)]">
                        <div className="flex gap-4 mb-6">
                            ...
                            <div>
                                <h3 className="font-serif text-xl font-bold m-0 mb-2 text-foreground">Delete Category?</h3>
                                <p className="text-sm leading-relaxed m-0 text-admin-muted">
                                    <strong className="text-foreground">"{toDelete.name}"</strong> will be permanently removed. This cannot be undone.
                                </p>
                            </div>
                        </div>
