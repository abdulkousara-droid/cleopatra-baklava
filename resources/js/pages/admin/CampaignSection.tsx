...
export default function CampaignSection({ subscriberCount, err, showToast }: {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/admin', {
            onSuccess: () => { clearErrors(); reset(); showToast(t('admin.toast_campaign_sent')); },
            onError: () => showToast(t('admin.toast_campaign_failed')),
        });
    };
        ...
        <>
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h2 className="font-serif text-[32px] font-bold m-0 leading-tight flex items-center gap-3 text-foreground">
                        <Send size={28} className="text-accent" /> {t('admin.campaign_title')}
                    </h2>
                    <p className="text-sm mt-1.5 text-admin-muted">
                        {t('admin.campaign_desc', { count: subscriberCount })}
                    </p>
                </div>
                <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold tracking-[0.1em] uppercase bg-accent/10 text-primary border border-accent/30">
                    <Users size={16} /> {t('admin.subscribers', { count: subscriberCount })}
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-white rounded-xl p-7 border border-admin-border">
                    <div className="mb-6">
                        <label className={labelClass}>{t('admin.subject_line')}</label>
                    ...
                    <div className="grid grid-cols-2 gap-5 mb-6">
                        <div>
                            <label className={labelClass}>
                                {t('admin.cta_button_text')} <span className="font-normal normal-case tracking-normal text-admin-faint">{t('admin.optional')}</span>
                            </label>
                        ...
                        <div>
                            <label className={labelClass}>
                                {t('admin.cta_link')} <span className="font-normal normal-case tracking-normal text-admin-faint">{t('admin.optional')}</span>
                            </label>
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-admin-border">
                        ...
                        <button type="submit" disabled={processing}
                            ...
                            <Send size={15} /> {t('admin.send_to_all')}
                        </button>
                    </div>
                </div>
            </form>
